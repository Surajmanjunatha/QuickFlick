...
const AddShows = () => {
  ...
  return nowPlayingMovies.length > 0 ? (
    ...
    <div className='overflow-x-scroll scroll-smooth pb-4 custom-scrollbar snap-x snap-mandatory'>
    ...
    {nowPlayingMovies.map((movie) =>(
      <div key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:-translate-y-1 transition duration-300 `} onClick={()=> setSelectedMovie(movie.id)}>
        <div className='relative w-40 h-60 rounded-lg overflow-hidden bg-gray-800'>
          <img src={image_base_url + movie.poster_path} alt={movie.title} className='w-full h-full object-cover brightness-90' />
    ...
    <div className='mt-8'>
      ...
      <div className='inline-flex items-center gap-2 border border-gray-600 px-3 py-2 rounded-md'>
        ...
        <div className='mt-6'>
          ...
          <input type="datetime-local" value={dateTimeInput} onChange={(e) => setDateTimeInput(e.target.value)} className='border border-gray-600 px-3 py-2 rounded-md bg-transparent text-white cursor-pointer' />
          <button onClick={handleDateTimeAdd} className='bg-primary/80 ml-3 text-white px-3 py-2 text-sm rounded-lg hover:bg-primary cursor-pointer'>Add Time</button>
        </div>
